...
Hexahedra Tree::RemoveHangingNodesMethod4() {
    ...
    for (size_t level = 0; level < c.size(); ++level) {
        ...
        for (size_t cLayerIdx = 0; cLayerIdx < cLayer.size(); ++cLayerIdx) {
            ...
            // 101000000101
            auto fourBlackEdges3 = [&](const std::array<std::array<double, 3>, 8>& positions) {
                std::array<size_t, 16> ptsIdx;
                ...
                ptsIdx[8] = addPoint(interpolatePoint(positions[0], 2, positions[1], 1));
                ptsIdx[9] = addPoint(interpolatePoint(positions[1], 2, positions[0], 1));
                ptsIdx[10] = addPoint(interpolatePoint(positions[2], 2, positions[3], 1));
                ptsIdx[11] = addPoint(interpolatePoint(positions[3], 2, positions[2], 1));
                ptsIdx[12] = addPoint(interpolate3Points(positions[4], threeWeights[0], positions[1], threeWeights[1], positions[0], threeWeights[2]));
                ptsIdx[13] = addPoint(interpolate3Points(positions[5], threeWeights[0], positions[0], threeWeights[1], positions[1], threeWeights[2]));
                ptsIdx[14] = addPoint(interpolate3Points(positions[6], threeWeights[0], positions[3], threeWeights[1], positions[2], threeWeights[2]));
                ptsIdx[15] = addPoint(interpolate3Points(positions[7], threeWeights[0], positions[2], threeWeights[1], positions[3], threeWeights[2]));
                // 4 hex elements
                hE.push_back({ ptsIdx[8], ptsIdx[9], ptsIdx[10], ptsIdx[11], ptsIdx[12], ptsIdx[13], ptsIdx[14], ptsIdx[15] });
                oneBlackEdge({ hX[ptsIdx[6]], hX[ptsIdx[2]], hX[ptsIdx[1]], hX[ptsIdx[5]], hX[ptsIdx[14]], hX[ptsIdx[10]], hX[ptsIdx[9]], hX[ptsIdx[13]] });
                oneBlackEdge({ hX[ptsIdx[4]], hX[ptsIdx[0]], hX[ptsIdx[3]], hX[ptsIdx[7]], hX[ptsIdx[12]], hX[ptsIdx[8]], hX[ptsIdx[11]], hX[ptsIdx[15]] });
                twoBlackEdges({ hX[ptsIdx[7]], hX[ptsIdx[6]], hX[ptsIdx[5]], hX[ptsIdx[4]], hX[ptsIdx[15]], hX[ptsIdx[14]], hX[ptsIdx[13]], hX[ptsIdx[12]] });
                };
            ...
            // T4
                ...
                else if (edgeIsSubdivided[0] && edgeIsSubdivided[2] && edgeIsSubdivided[9] && edgeIsSubdivided[11]) {
                    fourBlackEdges3(corners);
                    continue;
                }
                else if (edgeIsSubdivided[1] && edgeIsSubdivided[3] && edgeIsSubdivided[8] && edgeIsSubdivided[10]) {
                    fourBlackEdges3({ corners[1], corners[2], corners[3], corners[0], corners[5], corners[6], corners[7], corners[4] });
                    continue;
                }
                else if (edgeIsSubdivided[0] && edgeIsSubdivided[6] && edgeIsSubdivided[7] && edgeIsSubdivided[8]) {
                    fourBlackEdges3({ corners[4], corners[5], corners[1], corners[0], corners[7], corners[6], corners[2], corners[3] });
                    continue;
                }
                else if (edgeIsSubdivided[2] && edgeIsSubdivided[4] && edgeIsSubdivided[5] && edgeIsSubdivided[10]) {
                    fourBlackEdges3({ corners[5], corners[1], corners[0], corners[4], corners[6], corners[2], corners[3], corners[7] });
                    continue;
                }
                else if (edgeIsSubdivided[3] && edgeIsSubdivided[5] && edgeIsSubdivided[6] && edgeIsSubdivided[11]) {
                    fourBlackEdges3({ corners[0], corners[3], corners[7], corners[4], corners[1], corners[2], corners[6], corners[5] });
                    continue;
                }
                else if (edgeIsSubdivided[1] && edgeIsSubdivided[4] && edgeIsSubdivided[7] && edgeIsSubdivided[9]) {
                    fourBlackEdges3({ corners[4], corners[0], corners[3], corners[7], corners[5], corners[1], corners[2], corners[6] });
                    continue;
                }
            }
            // T6
