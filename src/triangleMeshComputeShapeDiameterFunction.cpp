...
#pragma omp parallel for
    for (long long fIdx = 0; fIdx < static_cast<long long>(f.size()); ++fIdx) {
        ...
        const std::array<double, 3> faceCenter = { v0[0] / 3 + v1[0] / 3 + v2[0] / 3,
            v0[1] / 3 + v1[1] / 3 + v2[1] / 3, v0[2] / 3 + v1[2] / 3 + v2[2] / 3 };
        ...
        const double crossValLen = std::sqrt(crossVal[0] * crossVal[0] +
            crossVal[1] * crossVal[1] + crossVal[2] * crossVal[2]);
        const std::array<double, 3> normal = { crossVal[0] / crossValLen,
            crossVal[1] / crossValLen, crossVal[2] / crossValLen };
        ...
        const std::array<double, 3> uRaw = { arbitraryVec[1] * normal[2] - arbitraryVec[2] * normal[1],
        ...
        const std::array<double, 3> v = { normal[1] * u[2] - normal[2] * u[1],
        ...
        for (size_t i = 0; i < numRays; ++i) {
            ...
            std::array<double, 3> ray = { normal[0] * normScale + u[0] * dx + v[0] * dy,
                normal[1] * normScale + u[1] * dx + v[1] * dy, normal[2] * normScale + u[2] * dx + v[2] * dy };
            const double rayLen = std::sqrt(ray[0] * ray[0] + ray[1] * ray[1] + ray[2] * ray[2]);
            ray[0] /= rayLen;
            ray[1] /= rayLen;
            ray[2] /= rayLen;
            // in the implementation, the weights are all 1
            rayResults[i] = std::make_pair(bVH.ClosestIntersection(faceCenter, ray, static_cast<size_t>(fIdx)), sample[2]);
        }
        ...
        const double threshold = medianDeviation * mADFactor;
